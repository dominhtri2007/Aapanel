...
echo = Bắt đầu Chặn địa chỉ ip phổ biến (Để bảo vệ) =
...
echo =====Đang tải xuống các khối IP từ rest.db.ripe.net =====
for i in "${!providers[@]}"
  do
  echo [+] Đang tải xuống IP for phạm vi địa chỉ hiện tại for "${providers[$i]}". Trích xuất cả IPv4 và IPv6
...
echo ===== Đang tải xuống các khối IP từ whois.arin.net =====
for i in "${!providers[@]}"
  do
...
echo ===== Đang tải xuống các khối IP từ bgpview.io =====
for i in "${!providers[@]}"
  do
  sleep 5
  echo [+] Downloading IPs for current address range for "${providers[$i]}". Extracting IPv4
...
echo " }" >> `pwd`/ngix_block.conf;
echo "}" >> `pwd`/ngix_block.conf;
echo [+] Có thể tìm thấy danh sách khối NGINX in `pwd`/ngix_block.conf
echo [+] Move `pwd`/ngix_block.conf to /etc/nginx/conf.d/ thư mục và khởi động lại nginx

echo ===== Tạo Apache mod_rewrite .htaccess =====
echo "[!] Lưu ý rằng bạn cần kích hoạt viết lại với a2enmod viết lại và khởi động lại máy chủ apache"
touch `pwd`/mod_rewrite.htaccess
echo "Viết LạiEngine On" >> `pwd`/mod_rewrite.htaccess;
while read line; do
echo "viết lại điều kiện expr \"-R '$line'\"" >> `pwd`/mod_rewrite.htaccess;
done < `pwd`/ipset_ip_extract.txt
echo "RewriteRule ^/(.*)?$ http://www.google.com/$1 [R=301,NC,NE,L]" >> `pwd`/mod_rewrite.htaccess;
echo [+] Có thể tìm thấy danh sách khối mod_rewrite của Apache in `pwd`/mod_rewrite.htaccess;
echo [+] Hãy nhớ bật ghi lại trong cấu hình apache toàn cầu in bằng cách thêm AllowOverride All vào thư mục bạn đang bảo vệ.
...
echo -ne '##################################################################################################   (100%)\r'
echo -ne '\n'
echo -ne 'Cài đặt thành công shild cơ bản'
echo -ne 'đã chặn: search.censys.io'
echo -ne 'Đề xuất thiết lập proxy CloudFlair để bảo vệ'
